...
os.makedirs('../public/tech', exist_ok=True)
...
# Download each icon
for url, filename in icons.items():
    print(f"Downloading {filename}...")
    try:
        urllib.request.urlretrieve(url, f'../public/tech/{filename}')
    except Exception as e:
        print(f"Error downloading {filename}: {e}")

# Create placeholder SVGs for missing icons
openai_svg = '''<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
  <circle cx="12" cy="12" r="10"/>
  <path d="M12 16v-4M12 8h.01"/>
</svg>'''

huggingface_svg = '''<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
...
with open('../public/tech/openai.svg', 'w') as f:
    f.write(openai_svg)

with open('../public/tech/huggingface.svg', 'w') as f:
